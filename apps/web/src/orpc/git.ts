...
export const connectionStatus = authedProcedure.handler(async ({ context }) => {
  ...
  return {
    ...
    githubAppSource: app ? (appInDb ? ("database" as const) : ("env" as const)) : null,
...
export const githubAppManifestStart = authedProcedure.handler(async () => {
  const publicUrl = platformConfig.publicControlPlaneUrl
  const { isPublicInternetUrl } = await import("@/lib/core")
...
export const removeGitLabOAuth = authedProcedure.handler(async () => {
