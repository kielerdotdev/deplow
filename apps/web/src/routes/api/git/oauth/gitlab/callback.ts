  ...
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        const base = redirectBaseFromRequest(request, configured)
        ...
        try {
          // Must match the redirect_uri used in startOAuth (configured public URL).
          const redirectUri = `${configured}/api/git/oauth/gitlab/callback`
