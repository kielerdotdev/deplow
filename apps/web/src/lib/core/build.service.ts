...
export class BuildService {
  ...
  async buildFromSource(input: {
    ...
    const strategy = selectBuildStrategy({
      sourcePath,
      hasDockerfile: detectDockerfile(sourcePath),
    })
    const image = this.imageTag(input.projectSlug, input.deploymentId)

    if (strategy === "dockerfile") {
      return this.buildWithDockerfile(sourcePath, image)
    }
    return this.buildWithRailpack(sourcePath, image)
  }
  ...
  private async buildWithDockerfile(
    ...
    return { strategy: "dockerfile", image, logs: formatLogs("dockerfile", result) }
  }
  ...
  private async buildWithRailpack(
    ...
    return { strategy: "railpack", image, logs: formatLogs("railpack", result) }
  }
}
