...
export class GitService {
  ...
  async syncRepo(input: {
    ...
    if (existsSync(path.join(dest, ".git"))) {
      const fetched = await this.tryFetchAndReset(dest, input.branch, env, secrets)
  ...
  private buildAuthEnv(
    ...
    const username =
      auth.username ??
      defaultGitUsername(auth.provider ?? (host.includes("gitlab") ? "gitlab" : "github"))
  ...
  private async tryFetchAndReset(
    ...
    const fetch = await this.runCommand("git", ["fetch", "origin", branch], dest, env)
    ...
    if (reset.code !== 0) {
      throw new Error(
        redactSecrets(`git reset failed: ${reset.stderr || reset.stdout}`.trim(), secrets),
      )
    }
  ...
  private async cloneFresh(
    ...
    if (clone.code !== 0) {
      throw new Error(
        redactSecrets(`git clone failed: ${clone.stderr || clone.stdout}`.trim(), secrets),
      )
    }
    return { sourcePath: dest, logs: logs.filter(Boolean).join("\n") }
  }
...
function pushLog(
  ...
  logs.push(redactSecrets(result.stdout, secrets), redactSecrets(result.stderr, secrets))
}
