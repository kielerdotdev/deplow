...
export async function saveGitHubAppConfig(
  ...
  await upsertIntegration("github_app", encryptString(JSON.stringify(config), secretKey()))
}

export async function saveGitLabOAuthConfig(
  config: GitLabOAuthConfig,
): Promise<void> {
  await upsertIntegration("gitlab_oauth", encryptString(JSON.stringify(config), secretKey()))
}
...
export async function consumeOAuthState(
  state: string,
): Promise<{ userId: string; provider: string; returnTo: string | null } | null> {
