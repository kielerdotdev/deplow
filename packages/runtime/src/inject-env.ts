...
type S3ConfigSlice = {
  s3: { appEndpoint: string; region: string }
}
...
export function injectDeployEnv(
  ...
  return {
    ...
    S3_REGION: credentials.storage.region ?? s3?.region ?? "us-east-1",
  }
}
...
export function injectDeployEnvFromBindings(
  ...
  const s3 = "s3" in config && config.s3 ? config.s3 : undefined
  if (input.storage) {
    env.S3_ENDPOINT = s3?.appEndpoint ?? input.storage.endpoint ?? ""
    ...
    env.S3_REGION = input.storage.region ?? s3?.region ?? "us-east-1"
  }
  return env
}
