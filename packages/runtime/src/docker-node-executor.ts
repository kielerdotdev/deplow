...
export type DockerDeployOptions = DeployOptions & {
  ...
  /** Publish container port to a host port (random if publishPort unset) */
...
export class DockerNodeExecutor implements NodeExecutor {
  ...
  async deployApp(
    ...
    const publishPort =
      options.publishPort ??
      (options.publishHostPort ? 0 : undefined)
    const { exposed, portBindings } = buildPortMappings(
      containerPort,
      publishPort,
      options.publishHostPort,
    )
    ...
    if (options.publishHostPort || options.publishPort) {
      publishedPort = await this.resolvePublishedPort(
        container.id,
        containerPort,
      )
    }
    return { containerId: container.id, serviceName, publishedPort }
  }
...
function buildPortMappings(
  ...
  if (publishPort !== undefined || publishHostPort) {
    // Empty HostPort = Docker assigns an ephemeral host port
    const host =
      publishPort && publishPort > 0 ? String(publishPort) : ""
    portBindings[`${containerPort}/tcp`] = [{ HostPort: host }]
  }
